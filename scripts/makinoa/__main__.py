...
start_at = iniparser.get("main", "start-at", fallback="0x08800000")
...
try:
    ...
    print(f"Error :: {start_at} is not a hexadecimal integer.")
    sys.exit(1)
...
if libgcc:
    ...
    with open(os.path.join(DEVKITARM, "lib", "gcc", "arm-none-eabi", version, "thumb", "libgcc.a"), "rb") as library:
...
if relocatable_objects:
    ...
    # TODO: Clean up
    sz = int(rslt.stdout.decode().split("\n")[1].split()[3])

    needed_words = round_up_to_4(sz + reserve) >> 2

offset_mask = 0x08000000
start_at |= offset_mask

start_at = round_up_to_4(start_at)

def find_needed_words(needed_words, start_at):
    if needed_words == 0:
        return 0
    
    with open("rom.gba", "rb") as rom:
        rom.seek(offset_mask ^ start_at)
...
with open("main.asm", "r", encoding="UTF-8") as instream:
    with open(os.path.join(build, "main.asm"), "w", encoding="UTF-8") as outstream:
        ...
        print(f".definelabel allocation, {find_needed_words(needed_words, start_at)}", file=outstream)
